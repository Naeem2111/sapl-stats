	...
	const fetchPlayerData = async () => {
		try {
			...
			const badgesResponse = await api.get(
				`/badges/player/${playerId}?season=${selectedSeason}`
			);
			...
			if (selectedSeason !== "all") {
				const statsResponse = await api.get(
					`/player-stats/player/${playerId}?season=${selectedSeason}`
				);
	...
	return (
		<div className="space-y-6">
			...
			<div className="card">
				...
				{/* Badge Categories */}
				<div className="space-y-6">
					{["PERFORMANCE", "ACHIEVEMENT", "MILESTONE", "SPECIAL"].map(
						(category) => {
													...
													<div className="flex-1 min-w-0">
														...
														{awardedBadge.match && (
															<div className="mt-1 text-xs text-gray-400">
																vs{" "}
																{awardedBadge.match.awayTeam?.name || "Unknown"}
															</div>
														)}
													</div>
												</div>
				...
				{badges.length === 0 && (
					<div className="text-center py-12">
						...
						<p className="mt-1 text-sm text-gray-500">
							This player hasn't earned any badges yet. Keep playing to unlock
							achievements!
						</p>
					</div>
				)}
